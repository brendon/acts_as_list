      ...
      module InstanceMethods
        ...
        private
          ...
          def decrement_positions_on_higher_items(position)
            acts_as_list_class.where("#{scope_condition} AND #{position_column} <= #{position}").each {|item|
              item.update_attributes(position_column => item.send(position_column) - 1)
            }
          end
          ...
          def decrement_positions_on_lower_items
            return unless in_list?
            acts_as_list_class.where("#{scope_condition} AND #{position_column} > #{send(position_column).to_i}").each {|item|
              item.update_attributes(position_column => item.send(position_column) - 1)
            }
          end
          ...
          def increment_positions_on_all_items
            acts_as_list_class.where("#{scope_condition}").each {|item|
              item.update_attributes(position_column => item.send(position_column) + 1)
            }
          end
